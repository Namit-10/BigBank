...
<li><a href="cardStatus.jsp" class="nav-link disabled">Card status</a></li>
...
<center>
	...
	<thead><tr><th>Application No</th><th>Account No</th><th>Applied Date</th><th>Approved Date</th><th>Application Status</th><th>Approve</th><th>Reject</th></tr></thead>
	<tbody>
	<% ResultSet rs2 = BankDao.showAllApplications();
		while(rs2.next()){ 
			if(rs2.getString(5).equals("PENDING")) {
		%>
			<tr><td><%= rs2.getString(1) %></td><td><%= rs2.getString(2) %></td><td><%= rs2.getDate(3) %></td><td><%= rs2.getDate(4) %></td><td><%= rs2.getString(5) %></td><td><form action='ApproveApplication' method='post'><input type="hidden" id="accountNo" name="accountNo" value = "<%= rs2.getString(2) %>"><input class = "btn btn-success" type="submit" value="Approve"></form></td><td><form action='RejectApplication' method='post'><input type="hidden" id="accountNo" name="accountNo" value = "<%= rs2.getString(2) %>"><input type="submit" class = "btn btn-danger" value="Reject"></form></td></tr>
		<%
		}
			else {
				%> 
				<tr><td><%= rs2.getString(1) %></td><td><%= rs2.getString(2) %></td><td><%= rs2.getDate(3) %></td><td><%= rs2.getDate(4) %></td><td><%= rs2.getString(5) %></td><td></td><td></td></tr>
