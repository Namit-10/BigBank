...
<% 	
	...
	if(accountNo.isEmpty()){
		response.sendRedirect("searchTransactionsByAccount.jsp");
	}
	HttpSession sess = request.getSession(); 
	ResultSet rs = (ResultSet)sess.getAttribute("adminResultSet");  
	String username = ""; 
	if(rs != null){ 
...
<li><a href="searchTransactionsByAccount.jsp" class="nav-link disabled">Specific Transactions</a></li>
...
<center>
	...
	<thead><tr><th>Transaction No.</th><th>Account No.</th><th>Date and Time</th><th>Transaction Type</th><th>Amount</th><th>Balance After Transaction</th></tr></thead>
	<tbody>
	<% ResultSet rs2 = BankDao.showTransactions(accountNo);
		while(rs2.next()){ 
		if(rs2.getString(4).equals("Dr")){ 
		%>
			<tr class="debit"><td><%= rs2.getInt(1) %></td><td><%= rs2.getString(2) %></td><td><%= rs2.getString(3) %></td><td><%= rs2.getString(4) %></td><td><%= rs2.getDouble(5) %></td><td><%= rs2.getDouble(6) %></td></tr>
		<%}
		else {%>
			<tr class="credit"><td><%= rs2.getInt(1) %></td><td><%= rs2.getString(2) %></td><td><%= rs2.getString(3) %></td><td><%= rs2.getString(4) %></td><td><%= rs2.getDouble(5) %></td><td><%= rs2.getDouble(6) %></td></tr>
		<%}
		}
	    %>
	 </tbody>
